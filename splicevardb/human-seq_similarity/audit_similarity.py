#!/usr/bin/env python3
"""
Audit potential train–test leakage between SpliceVarDB and risk datasets (MFASS/Vex-seq/MaPSy/ClinVar subsets).

Functions:
  1) Exact variant overlap: chrom:pos:ref:alt
  2) Positional overlap: chrom:pos (different allele)
  3) Optional sequence-similarity (SpanSeq-style) using ±window bp around variant and MinHash (k=21) Jaccard.

Inputs:
  - splicevardb TSV (e.g., splicevardb_filter_benchmark.tsv). Must contain either
    a) variant_id column (supports formats like 'chr1:1234:C>A' or '1:1234_C/A'), or
    b) columns chrom, pos, ref, alt.
  - One or more risk dataset files (TSV/CSV/VCF). Provide a small YAML/JSON mapping or CLI
    column flags to locate chrom/pos/ref/alt. VCF is auto-parsed.
  - Optional reference FASTA to extract ±window bp sequences for similarity screening.

Outputs (to --outdir):
  - summary.json: counts for exact/positional overlaps per dataset + optional similarity stats
  - exact_overlap_<name>.csv: intersecting variants
  - positional_overlap_<name>.csv: same position but allele differs
  - similarity_candidates_<name>.csv: pairs with MinHash Jaccard >= threshold (if FASTA provided)

Example:
  python audit_similarity.py \
    --spdb /path/splicevardb_filter_benchmark.tsv \
    --dataset name=MFASS file=/path/mfass_variants.tsv fmt=tsv chrom=chrom pos=pos ref=ref alt=alt \
    --dataset name=VexSeq file=/path/vexseq.vcf fmt=vcf \
    --fasta /path/GRCh38.fa --window 1000 --jaccard 0.9 \
    --outdir /tmp/audit
"""
...
def load_dataset_generic(file: str, fmt: str, mapping: Dict[str, str]) -> pd.DataFrame:
    ...
    for _, r in df.iterrows():
        row = {k: r[v] for k, v in cols.items()}
        # allow explicit mapping override
...
def exact_and_positional_overlap(a: pd.DataFrame, b: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # remove exact ones from positional using the same 'key'
...
def main():
    ...
    # Optional sequences for SpliceVarDB
    sp_seqs = None
    if args.fasta:
    ...
    for name, ds in datasets:
        ...
        if args.fasta:
            ...
            # iterate SP variants with positional pruning
            rows = []
            pos_window = args.window * 2  # conservative span for overlap pruning
            ...
            if args.blastn and rows:
                ...
                # only export sequences that appeared in candidates to keep BLAST small
                ...
                # run blastn (megablast-like defaults for high identity)
