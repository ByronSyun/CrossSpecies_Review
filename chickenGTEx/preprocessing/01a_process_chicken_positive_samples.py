...
def load_rsid_mapping(vcf_file_path):
    """Load rsID to genomic coordinate mapping from VCF file."""
    print(f"Loading rsID mapping from VCF: {vcf_file_path}")
    rsid_map = {}
            ...
            if len(parts) >= 5:
                ...
                if rsid.startswith('rs'):
                    ...
                    if line_count % 100000 == 0:
                        print(f"  Loaded {line_count:,} rsID mappings...")
        
        process.wait()
        print(f"Loaded {len(rsid_map):,} rsID mappings from VCF")
...
def process_positive_samples():
    """Process ChickenGTEx molQTL files to extract sQTL data."""
    ...
    for dir_path in possible_dirs:
        if os.path.isdir(dir_path):
            ...
            if sqtl_files:
                base_dir = dir_path
                print(f"Found sQTL data in: {base_dir}")
                break
    ...
    print(f"Starting ChickenGTEx positive sample processing...")
    
    vcf_file_path = './chicken_raw_data/gallus_gallus_variation.vcf.gz'
    rsid_map = load_rsid_mapping(vcf_file_path)
    
    if base_dir is None:
        print("ERROR: No sQTL data directory found. Run download_chicken_data.sh first.")
        return
    ...
    print(f"Found {len(sqtl_files)} sQTL files to process")
    ...
    if not all_positive_samples_dfs:
        print("ERROR: No sQTL data was successfully processed.")
        return

    print(f"Combining data from {len(all_positive_samples_dfs)} tissues...")
    ...
    if all(col in combined_df.columns for col in ['CHR', 'POS', 'REF', 'ALT', 'Tissue']):
        ...
        print(f"Aggregated into {len(agg_df):,} unique sQTLs from {combined_df['Tissue'].nunique()} tissues")
        agg_df.to_csv(output_file, sep='\t', index=False)
        print(f"Saved to: {output_file}")
        
        print(f"\nSummary: {len(agg_df):,} unique sQTL locations across {agg_df['CHR'].nunique()} chromosomes")
        
    else:
        combined_df.to_csv(output_file, sep='\t', index=False)
        print(f"Saved to: {output_file}")
        print(f"\nSummary: {len(combined_df):,} sQTL records")
