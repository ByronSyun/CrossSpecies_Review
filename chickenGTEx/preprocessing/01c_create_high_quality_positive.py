...
def create_high_quality_positive():
    """Create high-quality positive samples from ChickenGTEx lead sQTLs."""
    ...
    print("Creating high-quality ChickenGTEx positive samples...")
    ...
    try:
        ...
        print(f"Loaded {len(df):,} lead sQTL records")
    ...
    print(f"Loading reference genome...")
    try:
        ref_fasta = pysam.FastaFile(reference_file)
        available_chroms = ref_fasta.references
        print(f"Reference genome loaded ({len(available_chroms)} chromosomes)")
    except Exception as e:
    ...
    print(f"Processing {len(df):,} lead sQTLs...")
    ...
    for idx, row in df.iterrows():
        ...
        try:
            ...
            start_pos = max(1, pos - half_window)
            end_pos = min(chrom_length, pos + half_window)
            
            min_buffer = 1000
            if pos < min_buffer or pos > (chrom_length - min_buffer):
                errors['position_out_of_bounds'] += 1
                continue
            
            try:
                sequence = ref_fasta.fetch(chrom_final, start_pos-1, end_pos)
    ...
    print(f"Input: {len(df):,}, High-quality: {len(high_quality_samples):,}, Success rate: {len(high_quality_samples)/len(df)*100:.1f}%")
    print(f"Errors: {', '.join([f'{k}: {v}' for k, v in errors.items() if v > 0])}")
    
    if not high_quality_samples:
        print("ERROR: No high-quality samples were generated.")
        return
    ...
    print(f"Known tissue samples: {len(known_tissues_only):,} (filtered out {len(simplified_df) - len(known_tissues_only):,} unknown)")
    simplified_df = known_tissues_only
    
    print(f"\nBalanced sampling (target: 225,000)...")
    ...
    if total_samples <= target_total:
        ...
        print(f"Sampled balanced dataset: {len(final_df):,} samples")
    ...
    print(f"Summary: {len(output_df):,} samples, {output_df['variant_id'].nunique():,} unique variants, {len(output_df['tissue_id'].unique())} tissues")
