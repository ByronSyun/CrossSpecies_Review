...
def filter_lead_sqtls():
    """Filter ChickenGTEx sQTL data to retain only lead sQTLs per gene-tissue combination."""
    
    input_file = './processed_data/chicken_positive_samples_with_tissue.tsv'
    output_file = './processed_data/chicken_lead_sqtls.tsv'
    
    print("Starting lead sQTL filtering...")
    ...
    try:
        ...
        print(f"Loaded {len(df):,} sQTL records")
    ...
    if gene_col and pvalue_col:
        ...
        print(f"Filtered to {len(lead_sqtls):,} lead sQTLs (gene-tissue)")
        
    elif pvalue_col:
        df_sorted = df.sort_values(pvalue_col, ascending=True)
        lead_sqtls = df_sorted.groupby(['variant', 'tissue']).first().reset_index()
        print(f"Filtered to {len(lead_sqtls):,} lead sQTLs (variant-tissue)")
        ...
        print(f"After deduplication: {len(lead_sqtls):,} samples")
    
    initial_count = len(lead_sqtls)
    ...
    print(f"Summary: {len(lead_sqtls):,} lead sQTLs, {lead_sqtls['variant'].nunique():,} unique variants, {lead_sqtls['tissue'].nunique()} tissues")
