"""
AlphaGenome Predictor for ChickenGTEx Silver-Standard Benchmark (Binary Classification)
========================================================================================
This script runs zero-shot AlphaGenome predictions on the ChickenGTEx silver-standard 
binary classification dataset.

Core Logic:
  - Reads a TSV file containing 16384bp reference and alternate sequences.
  - As AlphaGenome is not pre-trained on the Gallus gallus genome, this script
    utilizes the full sequence context for predictions.
  - Performs batch predictions via the AlphaGenome API and calculates the max
    absolute difference between ref/alt scores as the prediction score.
"""
...
try:
    ...
    BASE_DIR = '/content/drive/MyDrive/Colab Notebooks/chickenGetx_classifier/'
...
INPUT_FILE = os.path.join(BASE_DIR, 'chickengtex_silver_benchmark_balanced_len16384.tsv')
...
# =============================================================================
class ChickenGTExBatchPredictor:
    """Handles batch prediction for the ChickenGTEx binary classification dataset."""
    ...
    def check_existing_batches(self):
        ...
        for i in range(NUM_BATCHES):
            batch_num = i + 1
            search_pattern = f"{self.batch_save_path}chickengtex_alphagenome_batch_{batch_num}_*.json"
    ...
    def save_batch_results(self, results, batch_num):
        ...
        batch_filename = f"chickengtex_alphagenome_batch_{batch_num}_{timestamp}.json"
    ...
    def merge_all_batches(self):
        ...
        for i in range(NUM_BATCHES):
            batch_pattern = f"chickengtex_alphagenome_batch_{i+1}_{timestamp}"
        ...
        if all_results:
            ...
            final_filename = f"chickengtex_alphagenome_complete_results_{final_timestamp}.json"
...
def run_batch_analysis():
    """Main function to coordinate the entire prediction workflow."""
    try:
        logging.info("[AlphaGenome] Initializing client and loading variants...")
        analyzer = ChickenGTExBatchPredictor(api_key=API_KEY, batch_save_path=BATCH_SAVE_PATH)
