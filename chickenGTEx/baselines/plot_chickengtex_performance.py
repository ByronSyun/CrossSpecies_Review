#!/usr/bin/env python3
"""
Plot ChickenGTEx performance results for all models
...
Note: Nucleotide_Transformer and SpliceBERT excluded due to poor human performance
"""
...
def load_evo2_zeroshot(tsv_path: str) -> pd.DataFrame:
    ...
    out = df[['variant_id', 'Evo2_zeroshot']].copy()
    out['variant_id'] = out['variant_id'].apply(normalize_variant_id)  # Normalize format
    return out.drop_duplicates('variant_id')
...
def load_dnabert2_logistic(tsv_path: str) -> pd.DataFrame:
    ...
    df = pd.read_csv(tsv_path, sep='\t')
    # Expected column: prob_splice_altering
    if 'prob_splice_altering' in df.columns:
        return df[['variant_id', 'prob_splice_altering']].rename(columns={'prob_splice_altering': 'DNABERT2_Logistic'}).drop_duplicates('variant_id')
...
def build_merged_dataframe(labels_path: str) -> pd.DataFrame:
    ...
    # Paths - Chicken excludes NT and SpliceBERT (poor human performance)
    ROOT = "/Users/byronsun/Desktop/AS_复现模型/BIB_review/code/chickenGTEx/baselines"

    alphagenome = load_alphagenome(f"{ROOT}/alphagenome/results/chickengtex_alphagenome_complete_results_20251018_205155.json")
    pangolin = load_pangolin(f"{ROOT}/Pangolin/results/pangolin_chickengtex_results.json")
    spliceai = load_spliceai(f"{ROOT}/SpliceAI/results/spliceai_parsed_scores_chicken.tsv")
    evo2_zero = load_evo2_zeroshot(f"{ROOT}/Evo2/results/evo2_chicken_predictions.tsv")
    evo2_mlp = load_evo2_mlp(f"{ROOT}/Evo2/results/evo2_mlp_chicken_predictions.tsv")
    dnabert2_log = load_dnabert2_logistic(f"{ROOT}/DNABert_2/results/dnabert2_chicken_logistic_scores.tsv")
    splicetransformer = load_splicetransformer(f"{ROOT}/spliceTransformer/results/SpliceTransformer_chickengtex_predictions.vcf")
    mmsplice = load_mmsplice(f"{ROOT}/mmsplice/results/mmsplice_chickengtex_scores.csv")

    # Merge (excluding NT and SpliceBERT)
    dfs = [
        alphagenome, evo2_zero, pangolin, spliceai,
...
def plot_roc_curves(df: pd.DataFrame, output_path: str) -> None:
    ...
    ax.set_title('ROC Curves - ChickenGTEx Cross-Species Benchmark', fontsize=24, fontweight='bold')
    ax.legend(loc='lower right', fontsize=20, frameon=True, fancybox=True, shadow=True, framealpha=0.95)
    ax.tick_params(axis='both', which='major', labelsize=18)
    ax.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(f"{output_path}/chicken_roc_curves.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_path}/chicken_roc_curves.pdf", bbox_inches='tight')
    plt.close()
...
def plot_precision_recall_curves(df: pd.DataFrame, output_path: str) -> None:
    ...
    ax.set_title('Precision-Recall Curves - ChickenGTEx Cross-Species Benchmark', fontsize=28, fontweight='bold')
    ...
    ax.set_ylim([0, 1.05])
    plt.tight_layout()
    plt.savefig(f"{output_path}/chicken_precision_recall_curves.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_path}/chicken_precision_recall_curves.pdf", bbox_inches='tight')
    plt.close()
...
def plot_performance_bar_chart(df: pd.DataFrame, output_path: str) -> None:
    ...
    ax.set_title('Model Performance Comparison - ChickenGTEx Cross-Species Benchmark', fontsize=16, fontweight='bold')
    ax.set_xticks(x)
    ...
    plt.tight_layout()
    plt.savefig(f"{output_path}/chicken_performance_comparison.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_path}/chicken_performance_comparison.pdf", bbox_inches='tight')
    plt.close()
...
def plot_coverage(df: pd.DataFrame, output_path: str) -> None:
    ...
    ax.set_title('Model Coverage - ChickenGTEx Cross-Species Benchmark', fontsize=16, fontweight='bold')
    ax.grid(True, alpha=0.3, axis='x')
    ax.set_xlim(0, 105)
    for bar, val in zip(bars, data):
        ax.text(val + 1, bar.get_y() + bar.get_height()/2, f'{val:.1f}%', va='center', ha='left', fontsize=11, fontweight='bold')
    plt.tight_layout()
    plt.savefig(f"{output_path}/chicken_coverage_analysis.png", dpi=300, bbox_inches='tight')
    plt.savefig(f"{output_path}/chicken_coverage_analysis.pdf", bbox_inches='tight')
    plt.close()


def main():
    labels_path = "/Users/byronsun/Desktop/AS_复现模型/BIB_review/data/processed_data/chickenGTEx/chickengtex_silver_benchmark_balanced.tsv"
    output_path = "/Users/byronsun/Desktop/AS_复现模型/BIB_review/code/chickenGTEx/baselines/visualisation"
    os.makedirs(output_path, exist_ok=True)

    print("📊 Building merged dataframe from individual model outputs (ChickenGTEx)...")
    print("   Note: Nucleotide_Transformer and SpliceBERT excluded (poor human performance)")
    ...
    merged_csv = "/Users/byronsun/Desktop/AS_复现模型/BIB_review/code/chickenGTEx/baselines/chickengtex_all_model_predictions.csv"
    ...
    print("✅ ChickenGTEx plots saved to:", output_path)
