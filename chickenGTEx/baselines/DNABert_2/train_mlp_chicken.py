#!/usr/bin/env python3
"""Train MLP Classifier on DNABERT-2 Concatenated Embeddings for Chicken Data"""
import numpy as np
import pandas as pd
...
def main(args):
    ...
    plt.title(f'DNABERT-2 MLP Chicken\nAUROC={auroc:.4f}, AUPRC={auprc:.4f}')
    ...
    print(f"Saved: model, scaler, metrics, confusion matrix, predictions")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train MLP on DNABERT-2 embeddings for chicken data")
    ...
    parser.add_argument("--confusion_matrix_file", type=str, required=True, help="Output confusion matrix")
