#!/usr/bin/env python3
"""
Run Pangolin predictions on ChickenGTEx data.

This script uses Pangolin to predict variant effects on the ChickenGTEx 
...
def check_environment():
    ...
    if not os.path.exists(CHICKEN_REFERENCE):
        missing_deps.append(f"Chicken reference genome: {CHICKEN_REFERENCE}")
    if not os.path.exists(CHICKEN_ANNOTATION):
        missing_deps.append(f"Chicken annotation database: {CHICKEN_ANNOTATION}")
    ...
    return True, PANGOLIN_ENV, CHICKEN_REFERENCE, CHICKEN_ANNOTATION
...
def run_pangolin_prediction(vcf_file, reference_genome, annotation_db, output_prefix, env_path=None):
    ...
    try:
        # Pangolin on ~25k variants can exceed 1 hour; extend timeout to 6 hours
        ...
        logging.error("Pangolin prediction timed out (6 hours)")
        return None
    ...
    try:
        ...
        output_data = {
            'metadata': {
                ...
                'model': 'Pangolin',
                'dataset': 'ChickenGTEx_Silver_Standard',
...
def main():
    parser = argparse.ArgumentParser(description="Run Pangolin predictions on ChickenGTEx data")
    ...
    if args.temp_dir:
        ...
        temp_prefix = os.path.join(args.temp_dir, "pangolin_chickengtex")
    else:
        temp_dir = tempfile.mkdtemp(prefix="pangolin_chickengtex_")
