#!/usr/bin/env python3
"""
Evaluate chickenGTEx consolidated predictions.
...
def main():
    base = Path('/Users/byronsun/Desktop/AS_复现模型/BIB_review/code/chickenGTEx/baselines')
    summary_file = base / 'chickengtex_all_model_predictions.csv'
    out_file = base / 'chickengtex_model_performance.csv'

    df = pd.read_csv(summary_file)
    y_true = df['ground_truth'].values.astype(int)

    # Evaluate 7 zero-shot models: 4 task-specific + 3 GFMs
    # Note: Nucleotide_Transformer and SpliceBERT skipped due to poor human performance (AUROC ~0.50-0.52)
    # Order: Task-specific first, then GFMs
    available_models = ['Pangolin', 'SpliceAI', 'MMSplice_pathogenicity', 'SpliceTransformer',
                        'AlphaGenome', 'Evo2_zeroshot', 'DNABERT2_Logistic']
    ...
    print('\nChickenGTEx Performance Summary:')
