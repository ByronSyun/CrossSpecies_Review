#!/bin/bash
# Pangolin PigGTEx Pipeline (Server Only)
# Converts pig TSV to VCF and runs Pangolin prediction
...
# Data paths
PIG_DATA="$BASE_DIR/data/processed_data/pigGTEx/piggtex_silver_benchmark_balanced.tsv"
SERVER_DATA="/mnt/userdata4/splicing/PigGTEx/processed_data/piggtex_silver_benchmark_balanced.tsv"

# Output paths
VCF_OUTPUT="$SCRIPT_DIR/piggtex_for_pangolin.vcf"
MAPPING_OUTPUT="$SCRIPT_DIR/piggtex_for_pangolin_mapping.csv"
PREDICTIONS_OUTPUT="$SCRIPT_DIR/results/pangolin_piggtex_results.json"

LOG_FILE="$SCRIPT_DIR/pangolin_pipeline_$(date +%Y%m%d_%H%M%S).log"
exec > >(tee -a "$LOG_FILE") 2>&1

echo "=== Pangolin PigGTEx Pipeline Started ==="
echo "Timestamp: $(date)"
echo "============================================="
...
if [[ "$IS_SERVER" == "true" ]]; then
    ...
    if [[ ! -f "$PIG_REFERENCE" ]]; then
        echo "❌ Pig reference genome not found: $PIG_REFERENCE"
        echo "Please download Sus_scrofa.Sscrofa11.1.dna.toplevel.fa to this location."
        exit 1
    fi
    
    PIG_ANNOTATION="/mnt/userdata4/splicing/PigGTEx/reference_genome/Sscrofa11.1.annotation.db"
    if [[ ! -f "$PIG_ANNOTATION" ]]; then
        echo "❌ Pig annotation database not found: $PIG_ANNOTATION"
...
echo ""
echo "Step 3: Converting PigGTEx TSV to VCF format..."
...
if [[ -f "$VCF_OUTPUT" && -s "$VCF_OUTPUT" && -f "$MAPPING_OUTPUT" && -s "$MAPPING_OUTPUT" ]]; then
    ...
    ${PYTHON_CMD} "$SCRIPT_DIR/convert_piggtex_to_vcf.py" \
        --input "$INPUT_DATA" \
        --output "$VCF_OUTPUT" \
...
if [[ -f "$PREDICTIONS_OUTPUT" && -s "$PREDICTIONS_OUTPUT" ]]; then
    ...
    echo "   Running Pangolin prediction (this may take up to 6 hours for ~26k variants)..."
    
    ${PYTHON_CMD} "$SCRIPT_DIR/predict_piggtex_pangolin.py" \
...
echo ""
echo "🎉 Pangolin PigGTEx Pipeline Completed Successfully!"
