#!/usr/bin/env python3
"""
Convert PigGTEx TSV data to VCF format for Pangolin input.

This script converts the PigGTEx silver-standard benchmark TSV file 
...
def convert_piggtex_to_vcf(input_tsv, output_vcf, sequence_length=8192):
    """
    Convert PigGTEx TSV to VCF format for Pangolin.
    ...
    """
    logging.info("=== PigGTEx to Pangolin VCF Conversion ===")
    ...
    logging.info("Loading PigGTEx data...")
    df = pd.read_csv(input_tsv, sep='\t', header=None)
    df.columns = ['variant_id', 'ref_sequence', 'alt_sequence', 'label', 'chrom']
    ...
    vcf_lines.append("##reference=Sscrofa11.1")
    vcf_lines.append("##source=PigGTEx_Silver_Standard")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Convert PigGTEx TSV to VCF format for Pangolin input"
    )
    ...
    try:
        convert_piggtex_to_vcf(args.input, args.output, args.sequence_length)
