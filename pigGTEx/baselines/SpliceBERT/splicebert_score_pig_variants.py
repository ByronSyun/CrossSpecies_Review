...
class SpliceBERTPigDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        full_ref_seq = row['ref_seq']  # Reference sequence from pig dataset
        ref_allele = row['ref']
        alt_allele = row['alt']
        
        # Truncate to SpliceBERT's maximum length (1024nt)
        # For pig data, the sequence is already 8192bp centered on the variant
...
def score_pig_variants(args):
    ...
    # Load pig dataset
    logging.info(f"Loading pig dataset from: {args.input_tsv}")
    ...
    logging.info(f"Loaded {len(df)} pig variants")

    # Create dataset
    dataset = SpliceBERTPigDataset(df, tokenizer, window_size=args.window_size)
...
def main():
    parser = argparse.ArgumentParser(description="Score pig variants using SpliceBERT")
    parser.add_argument('--input_tsv', required=True, help='Input TSV file with pig variants')
