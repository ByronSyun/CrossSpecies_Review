#!/usr/bin/env python3
"""
Merge ratGTEx ground truth with model predictions (zero-shot models only).
Outputs a consolidated CSV for downstream evaluation.

Inputs:
- Ground truth (balanced): ratgtex_silver_benchmark_balanced.tsv (no header)
- AlphaGenome TSV: variant_id, label, alphagenome_score
...
- SpliceTransformer JSON: { results: [ { variant_id, splicetransformer_score, label } ] }

Output:
- ratgtex_all_model_predictions.csv with columns:
  variant_id, ground_truth, AlphaGenome, Pangolin, SpliceAI, SpliceBERT, Evo2_zeroshot, Nucleotide_Transformer, MMSplice_pathogenicity, DNABERT2_Logistic, SpliceTransformer
...
def load_alphagenome(path: Path) -> pd.DataFrame:
    ...
    assert {'variant_id', 'alphagenome_score'}.issubset(df.columns), 'AlphaGenome TSV missing columns'
...
def main():
    ...
    gt_file = base_dir / 'data/processed_data/ratGTEx/ratgtex_silver_benchmark_balanced.tsv'
    ag_file = base_dir / 'code/ratGTEx/baselines/alphagenome/results/alphagenome_scores_20250915_142743(latest).tsv'
    pg_file = base_dir / 'code/ratGTEx/baselines/pangolin/results/pangolin_ratgtex_results.json'
    sa_file = base_dir / 'code/ratGTEx/baselines/SpliceAI/results/spliceai_parsed_scores_rat.tsv'
    sb_file = base_dir / 'code/ratGTEx/baselines/SpliceBERT/results/splicebert_rat_scores.csv'
    evo2_file = base_dir / 'code/ratGTEx/baselines/Evo2/results/evo2_rat_predictions.tsv'
    evo2_mlp_file = base_dir / 'code/ratGTEx/baselines/Evo2/results/evo2_mlp_rat_predictions.tsv'
    nt_file = base_dir / 'code/ratGTEx/baselines/Nucleotide Transformer/results/nt_rat_scores.csv'
    mmsplice_file = base_dir / 'code/ratGTEx/baselines/mmsplice/results/mmsplice_ratgtex_scores.csv'
    dnabert2_log_file = base_dir / 'code/ratGTEx/baselines/DNABert_2/results/dnabert2_rat_logistic_scores.tsv'
    dnabert2_mlp_file = base_dir / 'code/ratGTEx/baselines/DNABert_2/results/dnabert2_rat_mlp_variant_scores.tsv'
    st_file = base_dir / 'code/ratGTEx/baselines/spliceTransformer/SpliceTransformer_ratgtex_predictions.vcf'
    out_csv = base_dir / 'code/ratGTEx/baselines/ratgtex_all_model_predictions.csv'
